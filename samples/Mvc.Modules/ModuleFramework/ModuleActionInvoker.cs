...
{
    // This code is mostly copied from MVC's reflected action invoker. We don't have extensibility currently
    // to reuse the MVC invoker.
    public class ModuleActionInvoker : IActionInvoker
    {
        ...
        {
            _actionContext.Controller = _moduleFactory.CreateModule(_actionContext);
        ...
        internal static IActionResult CreateActionResult(object actionReturnValue)
        {
        ...
        private Task<IActionResult> InvokeActionMethod()
        {
            _cursor.SetStage(FilterStage.ActionMethod);

            var module = (MvcModule)_actionContext.Controller;

            var action = module.Actions[_descriptor.Index];
            var result = action.Func();

            return Task.FromResult(CreateActionResult(result));
        }
